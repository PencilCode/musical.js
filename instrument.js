...
var pianoTimbre = parseOptionString(
  "wave(sawtooth) volume(0.5) " +
  "attack(0.001) decay(0.7) sustain(0.01) release(0.1) " +
  "cutoff(2) resonance(1) detune(1.001)");
...
Instrument.prototype.now = function() {
  ...
  this._now = this._atop.ac.currentTime;
  return this._now;
}
    ...
    if (now < record.time + record.duration) {
      ...
      if (record.g) {
        var timbre = record.timbre || instrument._timbre,
            cleanuptime = now + timbre.release;
...
Instrument.prototype.makeNoteSound = function(record) {
  ...
  var timbre = record.timbre || this._timbre,
      ...
      amp = timbre.volume * record.velocity,
      ac = this._atop.ac,
      g, f, o;
  g = ac.createGain();
  ...
  o.start(starttime);
  o.stop(stoptime);
  if (timbre.detune && timbre.detune != 1.0) {
...
Instrument.prototype._doPoll = function() {
  ...
  for (j = 0; j < this._startSet.length; ++j) {
    if (this._startSet[j].time <= now) {
      this._trigger('stemstart', this._startSet[j]);
...
Instrument.prototype.schedule = function(delay, callback) {
  this._callbackSet.push(this.now() + delay, callback);
}
